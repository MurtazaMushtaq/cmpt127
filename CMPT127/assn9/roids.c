...
void ship_draw( const ship_t* ship )
{
  ...
  */
  for (int i = -1; i < 2; ++i)
  {
    for (int j = -1; j < 2; ++j)
    {
          draw_triangle( ship->x + dx+i, ship->y + dy+j,
             ship->x + lx+i, ship->y + ly+j,
             ship->x + rx+i, ship->y + ry+j,
             shipcolor );
...
void roid_draw( const roid_t* roid )
{
  ...
  */
  for (int i = -1; i < 2; ++i)
  {
    for (int j = -1; j<2; ++j)
    {
      draw_rectangle( roid->x-roid->width/2.0+i, roid->y-roid->height/2.0+j, 
      roid->x+roid->width/2.0+i, roid->y+roid->height/2+j,
      roid->color );
    }
  }
}
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
  ...
  for (int i = -1; i < 2; ++i)
  {
    for (int j = -1; j < 2; ++j)
    {
      return( shot->x +i >= roid->x +i - roid->width/2 && shot->x +i <= roid->x +i + roid->width/2 && shot->y +j >= roid->y +j - roid->height/2 && shot->y +j <= roid->y +j + roid->height/2);
    }
  }
}
